...
driftcalc <- function(datalist, splits){
  ...
  for(data in datalist){
    ...
    drift_by_condition <- rbind(drift_by_condition, select(thissub.pars, Drift, conditions, Sub.Num, num))
  }
...
ggplot(data = Facecut_drift, aes(y = Drift, x = conditions, group = conditions)) + geom_boxplot() + geom_jitter(width = .1, aes(size = num)) + xlab("Cutoff Point")
```
