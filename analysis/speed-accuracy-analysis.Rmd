...
idata <- merge(idata, df, by = c("Sub.Num", "Tr.Num")) %>%
...
## Scatterplot of RT vs dweltime of incorrect ASL and spoken responses
...
dweltime <- idata %>%
  filter(age_code.x == "child", is.na(RT.y) == F, correct == 0) %>%
...
for(rownum in 1:nrow(dweltime)){
  ...
  for(colnum in 10:100){
    ifelse(dweltime[rownum, colnum] == 0,
           ifelse(dweltime[rownum, "dwel"] > 0,
                  dweltime[rownum, "dwel"] <- dweltime[rownum, "dwel"],
                  dweltime[rownum, "dwel"] <-
                    as.numeric(colnames(dweltime)[colnum])),
           ifelse(dweltime[rownum, "dwel"] > 0,
                  dweltime[rownum, "dwel"] <-
                    as.numeric(colnames(dweltime)[colnum]) - dweltime[rownum, "dwel"],
                  dweltime[rownum, "dwel"] <- dweltime[rownum, "dwel"]))
  }
}

# graphing! Scatterplot with lines of best fit
fitASL <- lm(dwel ~ RT.y, data = filter(dweltime, language_modality.x == "ASL"))
fitEng <- (lm(dwel ~ RT.y, data = filter(dweltime, language_modality.x == "English")))
ggplot(dweltime, aes(x = RT.y, y = dwel, color = language_modality.x)) + geom_point() + geom_abline(intercept = fitASL$coefficients["(Intercept)"], slope = fitASL$coefficients["RT.y"], color = "red") + geom_abline(intercept = fitEng$coefficients["(Intercept)"], slope = fitEng$coefficients["RT.y"], color = "blue")
```