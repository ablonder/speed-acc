...
ASLFacecut <-
  ewma(ASLFace_ewma)/1000
## TODO - run DDM with ASLFacecut filter
...
filteredavgs_ASLFace <- df %>%
  filter(age_code == "child", is.na(RT) == F, stimuli == "ASL" | stimuli == "Face", RT_sec >= ASLFacecut) %>%
  group_by(stimuli) %>%
  summarize(avg = mean(correct, na.rm = T))
...
plot <- ggplot(timecourse, aes(x = time)) + geom_line(aes(y = C_D, color = "incorrect")) + geom_line(aes(y = C_T, color = "correct")) + geom_line(aes(y = off_center, color = "off center"))
plot + geom_hline(yintercept = 0, size = 1) + geom_hline(yintercept = 1, size = 1) + geom_hline(yintercept = .5, linetype = 2)

## TODO - separate graphs for correct and incorrect with separate lines for target, distractor, and center (see speed-acc-profile-plots)
## TODO - just include target and distractor, not center for ASL and Face
## TODO - calculate area between correct and incorrect after incorrect crosses .5 for all conditions (see Yurovsky Social Reference) - make curves first
```
...
plot <- ggplot(timecourse_ASL, aes(x = time)) + geom_line(aes(y = C_D, color = "incorrect")) + geom_line(aes(y = C_T, color = "correct")) + geom_line(aes(y = off_center, color = "off center")) + ylab(label = "Fixation")
plot + geom_hline(yintercept = 0, size = 1) + geom_hline(yintercept = 1, size = 1) + geom_hline(yintercept = .5, linetype = 2)
```
...
plot <- ggplot(timecourse_Face, aes(x = time)) + geom_line(aes(y = C_D, color = "incorrect")) + geom_line(aes(y = C_T, color = "correct")) + geom_line(aes(y = off_center, color = "off center")) + geom_line(aes(y = C_C, color = "Center to Center?")) + ylab("Fixation")
plot + geom_hline(yintercept = 0, size = 1) + geom_hline(yintercept = 1, size = 1) + geom_hline(yintercept = .5, linetype = 2)
```
```
