...
setwd("Documents/CSLI/speed-acc")
```
...
### Logistic plot of accuracy over reaction time

Not very informative
```{r}
ggplot(filter(df, age_code == "child", is.na(RT) == F),
       aes(x = RT, y = correct)) +
  geom_point() +
...
### Scatterplot of accuracy vs. reaction time by subject
```{r}
subj_acc <- df %>%
  filter(age_code == "child", is.na(RT) == F) %>%
  group_by(stimuli, Sub.Num) %>%
  summarize(accuracy = mean(correct), RT = mean(RT), num = n())

ggplot(subj_acc, aes(x = RT, y = accuracy, weight = num)) +
...
ewma <- function(data){
  ...
  results <- mutate(results, stimuli = data$stimuli)
...
ewma_graph = function(ewma_results){
  results <- ewma_results[["results"]]
  cutoffs <- ewma_results[["cutoff"]]
  plot <- ggplot(results, aes(rt)) +
     geom_point(aes(y = cs, color = "cs")) + geom_smooth(aes(y = cs)) +
     geom_line(aes(y = ucl, color = "UCL")) +
    facet_wrap(~stimuli) +
     geom_vline(data = cutoffs, aes(xintercept = cutoff), linetype = 2) +
...
ewma_graphs <- rbind(ASLcut$results, Facecut$results, Objcut$results, Bullcut$results)
cutoffs <- data.frame(rbind(c("ASL", ASLcut$cutoff*1000), c("Face", Facecut$cutoff*1000), c("Object", Objcut$cutoff*1000), c("Bullseye", Bullcut$cutoff*1000)))
colnames(cutoffs) <- c("stimuli", "cutoff")
cutoffs$stimuli <- as.factor(cutoffs$stimuli)
cutoffs$cutoff <- as.numeric(as.character(cutoffs$cutoff))

ewma_graphs <- list(results = ewma_graphs, cutoff = cutoffs)

ewma_graph(ewma_graphs)
```
...
## Predictions for the Follow-up Experiment

Bargraph of separation by condition - if Text is similar to Bullseye
```{r}
predict_saliency <- data.frame(rbind(c("Face", 1.65),
                                     c("Bullseye", 1.27),
                                     c("Text", (1.65+1.27)/2)))
colnames(predict_saliency) <- (c("condition", "sep"))

ggplot(predict_saliency, aes(x = condition, y = sep, fill = condition)) +
  geom_bar(stat = "identity") + guides(fill = F) +
  xlab("Condition") + ylab("Mean Boundary Separation") +
  theme(text = element_text(size = 30))
```

Bargraph of separation by condition - if Text is similar to ASL
```{r}
predict_info <- data.frame(rbind(c("Face", 1.65),
                                     c("Bullseye", 1.27),
                                     c("Text", 1.98)))
colnames(predict_info) <- (c("condition", "sep"))

ggplot(predict_info, aes(x = condition, y = sep, fill = condition)) +
  geom_bar(stat = "identity") + guides(fill = F) +
  xlab("Condition") + ylab("Mean Boundary Separation") +
  theme(text = element_text(size = 30))
```
