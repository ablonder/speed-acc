...
datalist <- list()
drift_by_cutoff <- data.frame(cutoff = numeric(), meandrift = numeric())
counter <- 0
for(cutoff in cuts){
  counter <- counter + 1
  datalist[[counter]] <- filter(Facedata, q >= cutoff)
  drift_by_cutoff[counter, "cutoff"] <- cutoff
}

driftcalc <- function(datalist, drift_by_cutoff){
  sub.pars <- data.frame(Separation = numeric(),
                         Non.Decision = numeric(),
                         Bias = numeric(),
                         Drift = numeric(),
  ...
  for(data in datalist){
    ...
    thisavg <- summarize(thissub.pars, mean = mean(Drift))
    drift_by_cutoff[counter, "meandrift"] <- thisavg[1, "mean"]
  }
  return(drift_by_cutoff)
}

Facecut_drift <- driftcalc(datalist = datalist,
                           drift_by_cutoff = drift_by_cutoff)

ggplot(Facecut_drift, aes(y = meandrift, x = cutoff)) + geom_point()
```
