...
## TODO - remove slow responses ie. after 2000 ms
## TODO - replace bargraphs with boxplots
```
...
```{r estpars}
dfiltered <- filter(d, (stimuli == "ASL" & q >= ASLcut['cutoff'])
                    | (stimuli == "Face" & q >= Facecut['cutoff']))
...
## TODO - vary Face cutoff and see how drift changes
```
...
```{r}
dfiltered_ASLFace <- filter(d, stimuli == "ASL" | stimuli == "Face", q >= ASLFacecut['cutoff'])
...
## TODO - pretty ddm/ewma results and to do
## TODO - set up rpubs
## TODO - pretty behavioral analyses (timecourse plots and the like)
## TODO - look into purrr for more abstract coding
```