...
```{r}
dfiltered_ASLFace <- filter(d, stimuli == "ASL" | stimuli == "Face", q >= ASLFacecut)
...
for(col in colnames(timecourse_alt)[11:100]){
  timeslot <- timecourse_alt %>%
    group_by(trial_type, timecourse_alt[,col]) %>%
    summarize(n())
  times <- merge(times, timeslot, by = c("trial_type", "timecourse_alt[,col]"))
}
